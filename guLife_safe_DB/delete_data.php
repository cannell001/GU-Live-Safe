...
if($action == 'shut-cancel'){
$shuttle_id=$_REQUEST['shuttle_id'];
echo $help_qry1="update  shuttle_services set shuttle_status='Cancelled' where shuttle_id=$shuttle_id";
$help1=mysql_query($help_qry1);
if($help1){
	header("Location:view_shuttle.php");
}else{
	header("Location:view_shuttle.php");
}
}if($action == 'knight-cancel'){
$gu_knights_id=$_REQUEST['gu_knights_id'];
$help_qry1s="update  shuttle_services set knight_status='Cancelled' where gu_knights_id=$gu_knights_id";
$help1=mysql_query($help_qry1s);
if($help2){
	header("Location:view_knight.php");
}else{
	header("Location:view_knight.php");
...
if($action == 'reportDelt'){
$report_id=$_REQUEST['report_id'];
$incident="delete from incident_reports  where report_id=$report_id";
$del_incident=mysql_query($incident);
if($del_incident){
